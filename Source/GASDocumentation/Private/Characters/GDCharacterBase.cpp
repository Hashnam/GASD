...
#include "Player/GDAnimInstance.h"
...
void AGDCharacterBase::PreInitializeComponents()
{
	Super::PreInitializeComponents();

	MainAnimInstance = Cast<UGDAnimInstance>(GetMesh()->GetAnimInstance());
	if (!MainAnimInstance)
	{
...
void AGDCharacterBase::SetupPlayerInputComponent(UInputComponent* PlayerInputComponent)
{
	...
	PlayerInputComponent->BindAxis("MoveForward/Backwards", this, &AGDCharacterBase::PlayerForwardMovementInput);
	PlayerInputComponent->BindAxis("MoveRight/Left", this, &AGDCharacterBase::PlayerRightMovementInput);
	PlayerInputComponent->BindAxis("LookUp/Down", this, &AGDCharacterBase::PlayerCameraUpInput);
	PlayerInputComponent->BindAxis("LookLeft/Right", this, &AGDCharacterBase::PlayerCameraRightInput);
	PlayerInputComponent->BindAction("JumpAction", IE_Pressed, this, &AGDCharacterBase::JumpPressedAction);
	PlayerInputComponent->BindAction("JumpAction", IE_Released, this, &AGDCharacterBase::JumpReleasedAction);
	PlayerInputComponent->BindAction("StanceAction", IE_Pressed, this, &AGDCharacterBase::StancePressedAction);
	PlayerInputComponent->BindAction("WalkAction", IE_Pressed, this, &AGDCharacterBase::WalkPressedAction);
	PlayerInputComponent->BindAction("RagdollAction", IE_Pressed, this, &AGDCharacterBase::RagdollPressedAction);
	PlayerInputComponent->BindAction("SelectRotationMode_1", IE_Pressed, this,
		&AGDCharacterBase::VelocityDirectionPressedAction);
	PlayerInputComponent->BindAction("SelectRotationMode_2", IE_Pressed, this,
		&AGDCharacterBase::LookingDirectionPressedAction);
	PlayerInputComponent->BindAction("SprintAction", IE_Pressed, this, &AGDCharacterBase::SprintPressedAction);
	PlayerInputComponent->BindAction("SprintAction", IE_Released, this, &AGDCharacterBase::SprintReleasedAction);
	PlayerInputComponent->BindAction("AimAction", IE_Pressed, this, &AGDCharacterBase::AimPressedAction);
	PlayerInputComponent->BindAction("AimAction", IE_Released, this, &AGDCharacterBase::AimReleasedAction);
	PlayerInputComponent->BindAction("CameraAction", IE_Pressed, this, &AGDCharacterBase::CameraPressedAction);
	PlayerInputComponent->BindAction("CameraAction", IE_Released, this, &AGDCharacterBase::CameraReleasedAction);
}


void AGDCharacterBase::PostInitializeComponents()
{
	Super::PostInitializeComponents();
	MyCharacterMovementComponent = Cast<UGDCharacterMovementComponent>(Super::GetMovementComponent());
}
...
void AGDCharacterBase::GetLifetimeReplicatedProps(TArray<FLifetimeProperty>& OutLifetimeProps) const
{
	...
	DOREPLIFETIME(AGDCharacterBase, TargetRagdollLocation);
	DOREPLIFETIME_CONDITION(AGDCharacterBase, ReplicatedCurrentAcceleration, COND_SkipOwner);
	DOREPLIFETIME_CONDITION(AGDCharacterBase, ReplicatedControlRotation, COND_SkipOwner);

	DOREPLIFETIME(AGDCharacterBase, DesiredGait);
	DOREPLIFETIME_CONDITION(AGDCharacterBase, DesiredStance, COND_SkipOwner);
	DOREPLIFETIME_CONDITION(AGDCharacterBase, DesiredRotationMode, COND_SkipOwner);

	DOREPLIFETIME_CONDITION(AGDCharacterBase, RotationMode, COND_SkipOwner);
	DOREPLIFETIME_CONDITION(AGDCharacterBase, OverlayState, COND_SkipOwner);
	DOREPLIFETIME_CONDITION(AGDCharacterBase, ViewMode, COND_SkipOwner);
	DOREPLIFETIME_CONDITION(AGDCharacterBase, VisibleMesh, COND_SkipOwner);
}
...
void AGDCharacterBase::RagdollStart()
{
...
void AGDCharacterBase::RagdollEnd()
{
...
void AGDCharacterBase::Server_SetMeshLocationDuringRagdoll_Implementation(FVector MeshLocation)
{
	TargetRagdollLocation = MeshLocation;
}

void AGDCharacterBase::SetMovementState(EALSMovementState NewState)
{
...
void AGDCharacterBase::SetMovementAction(EALSMovementAction NewAction)
{
...
void AGDCharacterBase::SetStance(EALSStance NewStance)
{
	if (Stance != NewStance)
	{
...
void AGDCharacterBase::SetGait(EALSGait NewGait)
{
	if (Gait != NewGait)
	{
...
void AGDCharacterBase::SetRotationMode(EALSRotationMode NewRotationMode)
{
...
void AGDCharacterBase::Server_SetRotationMode_Implementation(EALSRotationMode NewRotationMode)
{
	SetRotationMode(NewRotationMode);
}

void AGDCharacterBase::SetViewMode(EALSViewMode NewViewMode)
{
...
void AGDCharacterBase::Server_SetViewMode_Implementation(EALSViewMode NewViewMode)
{
	SetViewMode(NewViewMode);
}

void AGDCharacterBase::SetOverlayState(EALSOverlayState NewState)
{
...
void AGDCharacterBase::Server_SetOverlayState_Implementation(EALSOverlayState NewState)
{
	SetOverlayState(NewState);
}

void AGDCharacterBase::EventOnLanded()
{
	...
	else
	{
		...
		GetWorldTimerManager().SetTimer(OnLandedFrictionResetTimer, this,
			&AGDCharacterBase::OnLandFrictionReset, 0.5f, false);
	}
}
void AGDCharacterBase::Multicast_OnLanded_Implementation()
{
...
void AGDCharacterBase::EventOnJumped()
{
...
void AGDCharacterBase::Server_PlayMontage_Implementation(UAnimMontage* Montage, float PlayRate)
{
...
void AGDCharacterBase::Multicast_PlayMontage_Implementation(UAnimMontage* Montage, float PlayRate)
{
	if (!IsLocallyControlled())
	{
		MainAnimInstance->Montage_Play(Montage, PlayRate);
...
void AGDCharacterBase::Multicast_OnJumped_Implementation()
{
	if (!IsLocallyControlled())
	{
		EventOnJumped();
...
void AGDCharacterBase::Server_RagdollStart_Implementation()
{
	Multicast_RagdollStart();
}

void AGDCharacterBase::Multicast_RagdollStart_Implementation()
{
	RagdollStart();
}


void AGDCharacterBase::ReplicatedRagdollEnd()
{
	if (HasAuthority())
	{
...
void AGDCharacterBase::Server_RagdollEnd_Implementation(FVector CharacterLocation)
{
	Multicast_RagdollEnd(CharacterLocation);
}

void AGDCharacterBase::Multicast_RagdollEnd_Implementation(FVector CharacterLocation)
{
	RagdollEnd();
}

void AGDCharacterBase::ReplicatedRagdollStart()
{
	if (HasAuthority())
	{
		Multicast_RagdollStart();
...
void AGDCharacterBase::SetDesiredStance(EALSStance NewStance)
{
...
void AGDCharacterBase::Server_SetDesiredStance_Implementation(EALSStance NewStance)
{
	SetDesiredStance(NewStance);
}

void AGDCharacterBase::SetDesiredGait(EALSGait NewGait)
{
	DesiredGait = NewGait;
	if (GetLocalRole() == ROLE_AutonomousProxy)
	{
...
void AGDCharacterBase::Server_SetDesiredGait_Implementation(EALSGait NewGait)
{
	SetDesiredGait(NewGait);
}

void AGDCharacterBase::SetDesiredRotationMode(EALSRotationMode NewRotMode)
{
...
void AGDCharacterBase::Server_SetDesiredRotationMode_Implementation(EALSRotationMode NewRotMode)
{
	SetDesiredRotationMode(NewRotMode);
}


FVector AGDCharacterBase::GetPlayerMovementInput() const
{
...
void AGDCharacterBase::SetActorLocationAndTargetRotation(FVector NewLocation, FRotator NewRotation)
{
...
void AGDCharacterBase::SetHasMovementInput(bool bNewHasMovementInput)
{
...
FALSMovementSettings AGDCharacterBase::GetTargetMovementSettings() const
{
...
EALSGait AGDCharacterBase::GetAllowedGait() const
{
...
EALSGait AGDCharacterBase::GetActualGait(EALSGait AllowedGait) const
{
...
bool AGDCharacterBase::CanSprint() const
{
...
void AGDCharacterBase::OnBreakfall_Implementation()
{
	Replicated_PlayMontage(GetRollAnimation(), 1.35);
}


void AGDCharacterBase::Replicated_PlayMontage_Implementation(UAnimMontage* Montage, float PlayRate)
{
...
float AGDCharacterBase::GetAnimCurveValue(FName CurveName) const
{
	if (MainAnimInstance)
	{
...
void AGDCharacterBase::SetVisibleMesh(USkeletalMesh* NewVisibleMesh)
{
...
void AGDCharacterBase::Server_SetVisibleMesh_Implementation(USkeletalMesh* NewVisibleMesh)
{
	SetVisibleMesh(NewVisibleMesh);
}

void AGDCharacterBase::SetRightShoulder(bool bNewRightShoulder)
{
...
ECollisionChannel AGDCharacterBase::GetThirdPersonTraceParams(FVector& TraceOrigin, float& TraceRadius)
{
...
FTransform AGDCharacterBase::GetThirdPersonPivotTarget()
{
	return GetActorTransform();
}


FVector AGDCharacterBase::GetFirstPersonCameraTarget()
{
	return GetMesh()->GetSocketLocation(NAME_FP_Camera);
}


void AGDCharacterBase::GetCameraParameters(float& TPFOVOut, float& FPFOVOut, bool& bRightShoulderOut) const
{
...
void AGDCharacterBase::SetAcceleration(const FVector& NewAcceleration)
{
...
void AGDCharacterBase::SetIsMoving(bool bNewIsMoving)
{
...
FVector AGDCharacterBase::GetMovementInput() const
{
	return ReplicatedCurrentAcceleration;
}


void AGDCharacterBase::SetMovementInputAmount(float NewMovementInputAmount)
{
...
void AGDCharacterBase::SetSpeed(float NewSpeed)
{
	Speed = NewSpeed;
...
void AGDCharacterBase::SetAimYawRate(float NewAimYawRate)
{
...
void AGDCharacterBase::GetControlForwardRightVector(FVector& Forward, FVector& Right) const
{
...
void AGDCharacterBase::OnMovementModeChanged(EMovementMode PrevMovementMode, uint8 PreviousCustomMode /*= 0*/)
{
...
void AGDCharacterBase::OnMovementStateChanged(EALSMovementState PreviousState)
{
...
void AGDCharacterBase::OnMovementActionChanged(EALSMovementAction PreviousAction)
{
...
void AGDCharacterBase::OnStanceChanged(EALSStance PreviousStance)
{
...
void AGDCharacterBase::OnRotationModeChanged(EALSRotationMode PreviousRotationMode)
{
...
void AGDCharacterBase::OnGaitChanged(EALSGait PreviousGait)
{
...
void AGDCharacterBase::OnViewModeChanged(EALSViewMode PreviousViewMode)
{
...
void AGDCharacterBase::OnOverlayStateChanged(EALSOverlayState PreviousState)
{
	MainAnimInstance->OverlayState = OverlayState;
}


void AGDCharacterBase::OnVisibleMeshChanged(const USkeletalMesh* PreviousSkeletalMesh)
{
...
void AGDCharacterBase::OnStartCrouch(float HalfHeightAdjust, float ScaledHalfHeightAdjust)
{
...
void AGDCharacterBase::OnEndCrouch(float HalfHeightAdjust, float ScaledHalfHeightAdjust)
{
...
void AGDCharacterBase::Landed(const FHitResult& Hit)
{
	Super::Landed(Hit);
...
void AGDCharacterBase::OnLandFrictionReset()
{
...
void AGDCharacterBase::SetEssentialValues(float DeltaTime)
{
...
void AGDCharacterBase::UpdateCharacterMovement()
{
...
void AGDCharacterBase::UpdateGroundedRotation(float DeltaTime)
{
	if (MovementAction == EALSMovementAction::None)
	{
...
void AGDCharacterBase::UpdateInAirRotation(float DeltaTime)
{
	if (RotationMode == EALSRotationMode::VelocityDirection || RotationMode == EALSRotationMode::LookingDirection)
	{
...
void AGDCharacterBase::SmoothCharacterRotation(FRotator Target, float TargetInterpSpeed, float ActorInterpSpeed, float DeltaTime)
{
...
float AGDCharacterBase::CalculateGroundedRotationRate() const
{
...
void AGDCharacterBase::LimitRotation(float AimYawMin, float AimYawMax, float InterpSpeed, float DeltaTime)
{
...
void AGDCharacterBase::SetMovementModel()
{
...
void AGDCharacterBase::PlayerForwardMovementInput(float Value)
{
...
void AGDCharacterBase::PlayerRightMovementInput(float Value)
{
	if (MovementState == EALSMovementState::Grounded || MovementState == EALSMovementState::InAir)
	{
		// Default camera relative movement behavior
...
void AGDCharacterBase::PlayerCameraUpInput(float Value)
{
	AddControllerPitchInput(LookUpDownRate * Value);
}


void AGDCharacterBase::PlayerCameraRightInput(float Value)
{
	AddControllerYawInput(LookLeftRightRate * Value);
}


void AGDCharacterBase::JumpPressedAction()
{
...
void AGDCharacterBase::JumpReleasedAction()
{
	StopJumping();
}


void AGDCharacterBase::SprintPressedAction()
{
	SetDesiredGait(EALSGait::Sprinting);
}


void AGDCharacterBase::SprintReleasedAction()
{
	SetDesiredGait(EALSGait::Running);
}


void AGDCharacterBase::AimPressedAction()
{
...
void AGDCharacterBase::AimReleasedAction()
{
	if (ViewMode == EALSViewMode::ThirdPerson)
	{
		SetRotationMode(DesiredRotationMode);
...
void AGDCharacterBase::CameraPressedAction()
{
	UWorld* World = GetWorld();
	check(World);
	CameraActionPressedTime = World->GetTimeSeconds();
	GetWorldTimerManager().SetTimer(OnCameraModeSwapTimer, this,
		&AGDCharacterBase::OnSwitchCameraMode, ViewModeSwitchHoldTime, false);
}


void AGDCharacterBase::CameraReleasedAction()
{
...
void AGDCharacterBase::OnSwitchCameraMode()
{
	// Switch camera mode
	if (ViewMode == EALSViewMode::FirstPerson)
	{
...
void AGDCharacterBase::StancePressedAction()
{
...
void AGDCharacterBase::WalkPressedAction()
{
...
void AGDCharacterBase::RagdollPressedAction()
{
...
void AGDCharacterBase::VelocityDirectionPressedAction()
{
...
void AGDCharacterBase::LookingDirectionPressedAction()
{
...
void AGDCharacterBase::OnRep_RotationMode(EALSRotationMode PrevRotMode)
{
	OnRotationModeChanged(PrevRotMode);
}


void AGDCharacterBase::OnRep_ViewMode(EALSViewMode PrevViewMode)
{
	OnViewModeChanged(PrevViewMode);
}


void AGDCharacterBase::OnRep_OverlayState(EALSOverlayState PrevOverlayState)
{
	OnOverlayStateChanged(PrevOverlayState);
}


void AGDCharacterBase::OnRep_VisibleMesh(USkeletalMesh* NewVisibleMesh)
{
	OnVisibleMeshChanged(NewVisibleMesh);
}

void AGDCharacterBase::OnJumped_Implementation()
{
...
void AGDCharacterBase::BeginPlay()
{
	...
	DebugComponent = FindComponentByClass<UGDDebug>();
}


void AGDCharacterBase::RagdollUpdate(float DeltaTime)
{
...
void AGDCharacterBase::SetActorLocationDuringRagdoll(float DeltaTime)
{
	if (IsLocallyControlled())
	{
	...
	{
		UGDDebug::DrawDebugLineTraceSingle(World,
