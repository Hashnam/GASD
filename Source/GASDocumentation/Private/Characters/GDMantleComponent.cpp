...
#include "Characters/GDMantleComponent.h"
...
FName UGDMantleComponent::NAME_IgnoreOnlyPawn(TEXT("IgnoreOnlyPawn"));


UGDMantleComponent::UGDMantleComponent()
{
...
void UGDMantleComponent::BeginPlay()
{
	Super::BeginPlay();
	...
	if (GetOwner())
	{
		...
		if (OwnerCharacter)
		{
			...
			OwnerCharacter->JumpPressedDelegate.AddUniqueDynamic(this, &UGDMantleComponent::OnOwnerJumpInput);
			OwnerCharacter->RagdollStateChangedDelegate.AddUniqueDynamic(
				this, &UGDMantleComponent::OnOwnerRagdollStateChanged);

			DebugComponent = OwnerCharacter->FindComponentByClass<UGDDebug>();
		}
	}
}


void UGDMantleComponent::TickComponent(float DeltaTime, ELevelTick TickType,
...
void UGDMantleComponent::MantleStart(float MantleHeight, const FALSComponentAndTransform& MantleLedgeWS,
	EALSMantleType MantleType)
{
	...
	if (MantleType != EALSMantleType::LowMantle && OwnerCharacter->IsA(AGDHeroCharacter::StaticClass()))
	{
		Cast<AGDHeroCharacter>(OwnerCharacter)->ClearHeldObject();
	}
...
bool UGDMantleComponent::MantleCheck(const FALSMantleTraceSettings& TraceSettings, EDrawDebugTrace::Type DebugType)
{
	if (!OwnerCharacter)
	{
		return false;
	...
	FHitResult HitResult;
	{
		...
		{
			UGDDebug::DrawDebugCapsuleTraceSingle(World,
				TraceStart,
				TraceEnd,
...

	{
		...
		if (DebugComponent && DebugComponent->GetShowTraces())
		{
			UGDDebug::DrawDebugSphereTraceSingle(World,
				TraceStart,
				TraceEnd,
...
void UGDMantleComponent::Server_MantleStart_Implementation(float MantleHeight,
...
void UGDMantleComponent::Multicast_MantleStart_Implementation(float MantleHeight,
	const FALSComponentAndTransform& MantleLedgeWS,
	EALSMantleType MantleType)
{
...
void UGDMantleComponent::MantleUpdate(float BlendIn)
{
	if (!OwnerCharacter)
	{
		return;
...
void UGDMantleComponent::MantleEnd()
{
	...
	if (OwnerCharacter)
	{
		...
		if (OwnerCharacter->IsA(AGDHeroCharacter::StaticClass()))
		{
			Cast<AGDHeroCharacter>(OwnerCharacter)->UpdateHeldObject();
		}
	}
...
void UGDMantleComponent::OnOwnerJumpInput()
{
...
void UGDMantleComponent::OnOwnerRagdollStateChanged(bool bRagdollState)
{
